...
// -----
	...
	for (i in 1:n_ind) {
	  ...
	  X[i]       = beta_bd[1] + bd_ind[i] + beta_bd[2] * ind_hg[i];
	 ...
	 if (phi_zeros[t] == 1) {			// phi_zeros is 1 before an individual is caught for the first time
	  ...
	  if (phi_ones[t] == 1) {
	   ...
	   if (offseason[t] == 0) {
            ...
            phi[t] = inv_logit(beta_phi[1] + beta_phi[2] * X[ind_occ_min1_rep[t]] + beta_phi[3] * ind_size[ind_occ_min1_rep[t]] + beta_phi[4] * ind_hg[ind_occ_min1_rep[t]]);

	   } else {

            phi[t] = inv_logit(beta_offseason[1] + beta_offseason[2] * X[ind_occ_min1_rep[t]] + beta_offseason[3] * ind_size[ind_occ_min1_rep[t]] + beta_offseason[4] * ind_hg[ind_occ_min1_rep[t]]);
