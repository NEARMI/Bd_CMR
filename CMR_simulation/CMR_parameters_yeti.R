####
## Parameters for the CMR simulation, using values from a saved csv for running models on yeti
####
...
ind       <- matrix(data = yeti_val$pop_size
  , ncol = 1, nrow = n_pop)        
...
new_ind_per_pop <- rnbinom(n_pop, ind*0.1, .6)
...
times     <- matrix(data = rep(10, n_pop)
  , ncol = 1, nrow = n_pop)
...
for (i in 1:n_pop) {
 samp[[i]] <- rep(yeti_val$times, periods[i, 1])
}

## number of time periods that elapse between the on-season
between_season_duration <- matrix(data = 30
  , ncol = 1, nrow = n_pop)   
...
bd_theta  <- matrix(
  data = c(
    rep(yeti_val$ind_var, n_pop)  ## Intercept
...
p_mort          <- matrix(data = yeti_val$bd_eff
  , nrow = n_pop, ncol = 1)

background_mort <- matrix(data = rep(2, n_pop)
  , nrow = n_pop, ncol = 1)
