...
// Given long comments, this model is best read in full screen on an external monitor
  ...
  // long vector indices for observation model (p)
	...
	int<lower=0> p_year[ind_occ];		  	    // Vector designating periods for observational model (all occasions)
  ...
  // long vector indices for bd (bd)
	...
	int<lower=0> bd_time[ind_per_period_bd];
  ...
  // covariates
	...
	int<lower=0> n_ind_have;
	int<lower=0> n_ind_mis;
	int<lower=0> ind_len_which_have[n_ind_have];
	int<lower=0> ind_len_which_mis[n_ind_mis];
	real ind_len_have[n_ind_have];
	real<lower=0> ind_len_mean;
	real<lower=0> ind_len_sd;
...
// -----
	...
	vector[4] beta_inseason_year;			 // survival in season by year
	 ...
	 if (phi_zeros[t] == 1) {			// phi_zeros is 1 before an individual is caught for the first time
	  ...
	  if (phi_ones[t] == 1) {
	   ...
	   if (offseason[t] == 0) {
	     ...
	     phi[t] = inv_logit(
 beta_offseason_year[phi_year[t]] + 
 beta_offseason[1] * X[phi_bd_index[t]] + 
 beta_offseason[2] * ind_len[ind_occ_min1_rep[t]] +
