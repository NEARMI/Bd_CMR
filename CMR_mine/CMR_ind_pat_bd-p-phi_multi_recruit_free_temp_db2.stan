functions {
	...
	real prob_uncaptured(int n_occasions, real[] p, real[] phi) {

	vector[n_occasions] chi;    // chi for each capture date and individual 

        chi[n_occasions] = 1.0;     // on the last sampling date the probability is one
	...
	for (t in 1:(n_occasions - 1)) {   // loop over from the first to the second to last sampling date and multiply out the probabilities
         ...
         int t_next = t_curr + 1;
	 chi[t_curr] = (1 - phi[t_curr]) + phi[t_curr] * (1 - p[t_next - 1]) * chi[t_next];		
    }
...
data {
  ...
  // covariates
	int<lower=1> N_bd;				    // Number of defined values for bd
  	...
  	int<lower=1, upper=n_occasions> tt_bd[N_bd];        // occasion index that defines each bd entry
  	int<lower=0> X_bd[N_bd];			    // The bd values 
  ...
  // captures
	int<lower=1> N_y;				    // Number of defined values for captures
  	int<lower=1, upper=n_ind> ii_y[N_y];	            // individual index
  	int<lower=1, upper=n_occasions> tt_y[N_y];          // occasion index 
...
transformed parameters {
	...
	int ind_ind_f;
	int ind_ind_b;
	ind_ind_f = ((i - 1) * n_occ_min1 + 1);
	ind_ind_b = ((i - 1) * n_occ_min1 + first[i] - 1);

	for (zz in ind_ind_f:ind_ind_b) {
...
beta_phi[2]    * X[i, sampling_events[t]]
	...
	int ind_chi_f;
	int ind_chi_b;
	ind_chi_f = ((i - 1) * n_occasions + 1);
	ind_chi_b = ((i - 1) * n_occasions + n_occasions);

	  chi[ind_chi_f:ind_chi_b] = prob_uncaptured(n_occasions, p[ind_chi_f:ind_chi_b], phi[ind_chi_f:ind_chi_b]);
	}
}
