...
parameters {
	...
	real<lower=0> bd_sigma[3];			// individual variation in bd parameters (for now uncorrelated)
	matrix[n_ind, 3] bd_eps;			// the conditions modes of the random effect (each individual's slope in bd over time)
	
	real<lower=0> bd_add;    			// "process" noise for observed Bd compared to underlying state
...
transformed parameters {
	...
	vector[n_ind] bd_ind_int;
	matrix[n_ind, n_occasions] bd_ind_slope;   
	matrix[n_ind, n_occasions] bd_ind_sq_slope;                                 

	// individual variation in their bd trajectories from the grand mean over time

	for (i in 1:n_ind) {
  	  bd_ind_int[i]         = beta_bd[1] + (bd_sigma[1] * bd_eps[i, 1]);
  	  bd_ind_slope[i, ]     = (beta_bd[2] + bd_sigma[2] * bd_eps[i, 2]) * samp_events;
  	  bd_ind_sq_slope[i, ]  = (beta_bd[3] + bd_sigma[3] * bd_eps[i, 3]) * square(samp_events);
	}
...
model {
	...
	beta_p[1] ~ normal(0, 5);
	beta_p[2] ~ normal(0, 5);
	beta_bd[1] ~ normal(0, 1);
	beta_bd[2] ~ normal(0, 3);
	beta_bd[3] ~ normal(0, 3);

	bd_sigma[1] ~ inv_gamma(1, 1);
	bd_sigma[2] ~ inv_gamma(1, 1);
	bd_sigma[3] ~ inv_gamma(1, 1);

	for (i in 1:n_ind) {
	for (j in 1:3) {
	  bd_eps[i, j] ~ normal(0, 1);
	}
	}
	...
	// Bd "Process Model"
	 // Here just treating this as a regression where bd follows a quadratic over time 
	  // This is a bit funky because in reality individuals vary in when they get infected... (I guess being on the log scale makes
           // this vaguely ok as individuals that get infected later will have a large negative intercept)


	// Is there a more sensible way to write this? That is, is there a way to estimate the bd-survival relationship without having to completely model
	 // every single individual's latent bd state? No??	

        for (i in 1:n_ind) {
	 X[i, ] ~ normal(bd_ind_int[i] + bd_ind_slope[i, ] + bd_ind_sq_slope[i, ], bd_add);
	}
