...
parameters {
	...
	vector[n_ind] eps_alpha_phi;			// conditional modes of the random effect for survival
...
transformed parameters {
	...
	for (i in 1:n_ind) {
  	  bd_ind[i]  = bd_delta_mu + bd_delta_sigma  * bd_delta_eps[i];
          p_ind[i]   = beta_p[1]   + sigma_alpha_p   * eps_alpha_p[i];
	  phi_ind[i] = beta_phi[1] + sigma_alpha_phi * eps_alpha_phi[i];
	...
	// the individual survival and capture probabilities are based on an individuals nature (random effect of individual for intercept)
	  ...
	  phi[i, t] = inv_logit(phi_ind[i] + beta_phi[2] * X[i, t]);
	  p[i, t] = inv_logit(p_ind[i] + beta_p[2] * X[i, t]);
