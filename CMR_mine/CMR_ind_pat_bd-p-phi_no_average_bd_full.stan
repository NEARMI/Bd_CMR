...
data {
	...
	int<lower=0> sampling_events[n_times];
	int<lower=1> which_samples[n_occasions];	
	...
	int<lower=0,upper=n_occasions> time_gaps[n_times];
...
transformed parameters {
	...
	matrix<lower=0,upper=1>[n_ind, n_times] phi;	  // probability of surviving to the next period
	matrix<lower=0,upper=1>[n_ind, n_times] p;	  // probability of being captured in a period
	...
	chi = prob_uncaptured(n_ind, n_occasions, p[, which_samples], phi[, which_samples]);
}
