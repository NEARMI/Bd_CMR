...
data {
	...
	int<lower=0> periods;				    // Total number of primary periods over which individuals are captured
	int<lower=0> n_ind[periods];			    // Total number of individuals caught (ever, over all years)
	int<lower=2> n_times[periods];			    // Number of discrete time points in each season
	int<lower=2> n_occasions[periods];		    // Number of capture occasions on a subset of times
	int<lower=1> n_oc_min1[periods];		
