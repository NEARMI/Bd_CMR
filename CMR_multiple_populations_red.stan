	 ...
	 if (phi_zeros[t] == 1) {			// phi_zeros is 1 before an individual is caught for the first time
	  ...
	  if (phi_ones[t] == 1) {
	   ...
	   if (offseason[t] == 0) {
             ...
             phi[t] = inv_logit(beta_phi[1] + inseason_pop[pop_phi[t]] + beta_phi[2] * ind_size[ind_occ_min1_rep[t]]);

	   } else {

	     phi[t] = inv_logit(beta_offseason[1] + offseason_pop[pop_phi[t]] + 
(beta_offseason[2] + offseason_pop_bd[pop_phi[t]]) * X[phi_bd_index[t]] + 
...
// -----
	...
	for (t in 1:ind_occ) {
	 ...
	 if (p_zeros[t] == 1) {				
          p[t] = inv_logit(beta_p[1] + p_pop[pop_p[t]] + beta_p[2] * ind_size[ind_occ_rep[t]]);
	 } else {
          p[t] = inv_logit(beta_p[1] + p_pop[pop_p[t]] + beta_p[2] * ind_size[ind_occ_rep[t]]) * gamma[gamma_index[t]];
	 }
