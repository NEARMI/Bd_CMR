...
// -----
	...
	real<upper=0> beta_offseason_bd;
	 ...
	 if (phi_zeros[t] == 1) {			// phi_zeros is 1 before an individual is caught for the first time
	  ...
	  if (phi_ones[t] == 1) {
	   ...
	   if (offseason[t] == 0) {
            ...
            phi[t] = inv_logit(beta_offseason[1] + beta_offseason_bd * X[ind_occ_min1_rep[t]] + beta_offseason[2] * ind_size[ind_occ_min1_rep[t]]);
...
// -----
	...
	for (t in 1:ind_occ) {
	 ...
	 if (p_zeros[t] == 1) {				
          p[t] = inv_logit(beta_p[1] + beta_p[2] * X[ind_occ_rep[t]] + beta_p[3] * ind_size[ind_occ_rep[t]]);
	 } else {
          p[t] = inv_logit(beta_p[1] + beta_p[2] * X[ind_occ_rep[t]] + beta_p[3] * ind_size[ind_occ_rep[t]]) * gamma[gamma_index[t]];
	 }
...
// -----
	...
	beta_offseason_bd ~ normal(0, 1.2);
