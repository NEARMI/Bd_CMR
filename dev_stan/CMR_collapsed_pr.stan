...
// -----
	...
	for (i in 1:n_ind) {
	  ...
	  X[i]       = beta_bd[1] + bd_ind[i] + bd_pop[ind_in_pop[t]] + beta_bd[2] * ind_hg[i];
	 ...
	 if (phi_zeros[t] == 1) {			// phi_zeros is 1 before an individual is caught for the first time
	  ...
	  if (phi_ones[t] == 1) {
	   ...
	   if (offseason[t] == 0) {
            ...
            phi[t] = inv_logit(beta_phi[1] + inseason_pop[pop_phi[t]] + beta_phi[2] * X[ind_occ_min1_rep[t]] + beta_phi[3] * ind_size[ind_occ_min1_rep[t]] + beta_phi[4] * ind_hg[ind_occ_min1_rep[t]]);

	   } else {

            phi[t] = inv_logit(beta_offseason[1] + offseason_pop[pop_phi[t]] + beta_offseason[2] * X[ind_occ_min1_rep[t]] + beta_offseason[3] * ind_size[ind_occ_min1_rep[t]] + beta_offseason[4] * ind_hg[ind_occ_min1_rep[t]]);
...
// -----
	...
	for (t in 1:ind_occ) {
	 ...
	 if (p_zeros[t] == 1) {				
          p[t] = inv_logit(beta_p[1] + p_pop[pop_p[t]] + beta_p[2] * X[ind_occ_rep[t]] + beta_p[3] * ind_size[ind_occ_rep[t]] + beta_p[4] * ind_hg[ind_occ_rep[t]] + beta_p_y[periods_occ[t]] + beta_p_m[p_month[t]]);
	 } else {
          p[t] = inv_logit(beta_p[1] + p_pop[pop_p[t]] + beta_p[2] * X[ind_occ_rep[t]] + beta_p[3] * ind_size[ind_occ_rep[t]] + beta_p[4] * ind_hg[ind_occ_rep[t]] + beta_p_y[periods_occ[t]] + beta_p_m[p_month[t]]) * gamma[gamma_index[t]];
	 }
