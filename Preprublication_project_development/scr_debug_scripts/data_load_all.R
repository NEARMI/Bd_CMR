...
## Convert bd load to a numeric and drop all individuals that were never marked as they can't be used to inform the model
 ## For now also drop individuals found dead. Could add a flag in the model to indicate that we know that an individual
  ## died at some point between the last capture and the current capture of the dead individual, but this is kind of a hassle
...
data.temp %<>% mutate(bd_load = as.numeric(bd_load)) %>%
  ...
  filter(!is.na(Mark), Mark != "") ## --- && %>% filter(dead != 1 | is.na(dead))
...
############### You probably wont need the rest of this as it starts to pull together the captures with the site covaraites, but leaving it here
