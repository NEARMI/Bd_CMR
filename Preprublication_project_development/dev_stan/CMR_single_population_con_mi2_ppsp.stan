	 ...
	 if (phi_zeros[t] == 1) {	 // phi_zeros is 1 before an individual is caught for the first time
	  ...
	  if (phi_ones[t] == 1) {
	   ...
	   if (offseason[t] == 0) {	 // in season survival process
             ...
             phi[t] = inv_logit(beta_inseason_year[phi_year[t]]);
...
// -----
	...
	for (t in 1:ind_occ) {
	 ...
	 if (p_zeros[t] == 1) {				
          //  p[t] = inv_logit(beta_p_year[p_year[t]] + beta_p * p_effort[t]); // effort important usually (I THINK), but for my current exploration with JP_RALU effort is always 1
	      p[t] = inv_logit(beta_p_year[p_year[t]]);
	 } else {
          //  p[t] = inv_logit(beta_p_year[p_year[t]] + beta_p * p_effort[t]) * gamma[gamma_index[t]];
	      p[t] = inv_logit(beta_p_year[p_year[t]]) * gamma[gamma_index[t]];
	 }
