	 ...
	 if (phi_zeros[t] == 1) {			// phi_zeros is 1 before an individual is caught for the first time
	  ...
	  if (phi_ones[t] == 1) {
	   ...
	   if (offseason[t] == 0) {
            ...
            phi[t] = inv_logit(beta_phi[1] + beta_phi[2] * X[ind_occ_min1_rep[t]] + beta_phi[3] * ind_size[ind_occ_min1_rep[t]]);

	   } else {

            phi[t] = inv_logit(beta_offseason[1] + beta_offseason[2] * X[ind_occ_min1_rep[t]] + beta_offseason[3] * ind_size[ind_occ_min1_rep[t]]);
...
// -----
	...
	for (t in 1:ind_occ) {
	 ...
	 if (p_zeros[t] == 1) {				
          p[t] = inv_logit(beta_p[1] + beta_p[2] * X[ind_occ_rep[t]] + beta_p[3] * ind_size[ind_occ_rep[t]] + beta_p[4] * p_month[t] + beta_p_y[periods_occ[t]]);
	 } else {
          p[t] = inv_logit(beta_p[1] + beta_p[2] * X[ind_occ_rep[t]] + beta_p[3] * ind_size[ind_occ_rep[t]] + beta_p[4] * p_month[t] + beta_p_y[periods_occ[t]]) * gamma[gamma_index[t]];
	 }
