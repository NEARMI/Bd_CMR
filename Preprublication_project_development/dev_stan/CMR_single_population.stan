	...
	for (t in 1:ind_occ_min1) {
	 ...
	 if (phi_zeros[t] == 1) {			// phi_zeros is 1 before an individual is caught for the first time
	  ...
	  if (phi_ones[t] == 1) {
	   ...
	   if (offseason[t] == 0) {
             phi[t] = inv_logit(beta_phi[1] + beta_phi[2] * X[phi_bd_index[t]] + beta_phi[3] * ind_size[ind_occ_min1_rep[t]]);
	   } else {
	     phi[t] = inv_logit(beta_offseason_year[phi_year[t]] + beta_offseason[1] * X[phi_bd_index[t]] + beta_offseason[2] * ind_size[ind_occ_min1_rep[t]]);
	   }
