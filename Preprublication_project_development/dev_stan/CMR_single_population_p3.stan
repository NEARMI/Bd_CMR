...
// -----
 ...
 p_day_dev[p_day[t]] + 
 beta_p_len * ind_len_scaled[ind_occ_rep[t]] +
 beta_p_bd  * X[p_bd_index[t]]
);
	 }
	}

	for (t in 1:n_days) {
	  p_per_day[t] = inv_logit(ind_sex[1, ] * beta_p_sex + p_day_dev[t]);
	}
...
// Detection Priors
	...
	beta_p_len	  ~ normal(0, 0.75);
	beta_p_sex        ~ normal(0, 0.75);
	beta_p_bd         ~ normal(0, 0.75);

	p_day_delta_sigma ~ inv_gamma(8, 15);

	for (i in 1:n_days) {
	  p_day_delta_eps[i] ~ normal(0, 0.85);
	}
