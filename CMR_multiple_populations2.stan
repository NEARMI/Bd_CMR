...
// -----
	...
	real<lower=0> offseason_pop_year_sigma;		 // variation in offseason survival by population (intercept)
	real offseason_pop_year_eps[n_pop_year];
...
transformed parameters {
	...
	real offseason_pop_year[n_pop_year];
...
// -----
	...
	for (pp in 1:n_pop_year) {
	  ...
	  offseason_pop_year[pp] = offseason_pop_year_sigma * offseason_pop_year_eps[pp];
	} 
...
beta_phi[3] * ind_size[ind_occ_min1_rep[t]]);
	     ...
	     phi[t] = inv_logit(beta_offseason[1] + offseason_pop_year[phi_pop_year[t]] +
(beta_offseason[2] + offseason_pop[pop_phi[t]]) * X[phi_bd_index[t]] + 
