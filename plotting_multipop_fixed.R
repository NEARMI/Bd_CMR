...
spec_sex_mm.t <- spec_sex_mm.t[spec_sex$pop[k], ]
spec_slope    <- spec_sex_mm.t[-c(2:3)]
}
...
if (n_specs > 1) {
 ...
 pred.est[j, ] <- plogis(
    (sweep(stan.fit.samples$beta_offseason_int, 2, spec_sex_mm.t, `*`) %>% rowSums()) +
    (sweep(stan.fit.samples$beta_offseason_bd, 2, spec_slope, `*`) %>% rowSums()) * pred.vals$bd[j] +
    (sweep(stan.fit.samples$beta_offseason_len, 2, spec_slope, `*`) %>% rowSums()) * pred.vals$len[j] +
     stan.fit.samples$beta_offseason_mehg[, 1] * pred.vals$mehg[j]
 )
}
}
...
gg2 <- pred.vals.gg %>% filter(sex == "M", mehg == 0, bd == 0) %>% {
  ggplot(., aes(len, mid)) + 
    geom_ribbon(aes(ymin = lwr, ymax = upr, fill = pop), alpha = 0.3) +
    geom_line(aes(colour = pop), size = 1) + 
    scale_colour_discrete() +
