...
// Given long comments, this model is best read in full screen on an external monitor
  ...
  // covariates (length)
	...
	vector[n_ind_len_have] ind_len_have;		    // The actual length values that we have
 	int<lower=0> ind_len_spec_have[n_ind_len_have];	    // Species associated with each of the length values
 	int<lower=0> ind_len_spec_mis[n_ind_len_mis];  	    // Species associated with each of the length values that are missing
...
// -----
  ...
  // fixed
	...
	real beta_p_drawdown[3];			 // daily detection probably as a function of drawdown amount
	real beta_p_veg[4];			         // daily detection probably as a function of vegetation amount
...
// -----
  ...
  // fixed
	...
	real beta_mehg_drawdown[3];			 // effect of drawdown on MeHg
	real beta_mehg_hydro[2];			 // effect of hydro period on MeHg
	real beta_mehg_sub[1];				 // effect of substrate on MeHg
	real beta_mehg_region[3];			 // effect of region on MeHg
...
beta_mehg_hydro[pop_hydro[z]]       + 							// hydro effect on MeHg
beta_mehg_sub[pop_sub[z]]           + 							// substrate effect on MeHg
beta_mehg_region[pop_region[z]]     + 							// region effect on MeHg
...
beta_mehg_hydro[pop_hydro[ind_mehg_pop[i]]]       + 					// hydro effect on MeHg
beta_mehg_sub[pop_sub[ind_mehg_pop[i]]]           + 					// substrate effect on MeHg
beta_mehg_region[pop_region[ind_mehg_pop[i]]]     + 					// region effect on MeHg
...
// -----
  ...
  // fixed
	...
	beta_mehg_hydro    ~ normal(0, 1);
	beta_mehg_sub      ~ normal(0, 1);
	beta_mehg_region   ~ normal(0, 1);
...
// -----
  ...
  // values that we have (to constrain ind_len_alpha and beta)
	for (nih in 1:n_ind_len_have) {
	  ind_len_have[nih] ~ gamma(ind_len_alpha[ind_len_spec_have[nih]], ind_len_beta[ind_len_spec_have[nih]]);
	}

  // values that we don't have (drawn from the estimated ind_len_alpha and beta)
