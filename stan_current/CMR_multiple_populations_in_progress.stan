...
// Given long comments, this model is best read in full screen on an external monitor
  ...
  // short vector indexes (length of n_ind)	
	...
	int<lower=0> ind_which_pop[n_ind];		    // Which population each individual belongs to
  ...
  // site-level covariates, continuous
	real<lower=0> pop_temp[n_pop_year];		    // population*year specific covariate for temperature
  ...
  // captures
	...
	vector<lower=0>[num_days] n_capt_per_day;	    // Number of captures on each day 
...
//	vector<lower=0>[n_pop] p_day_delta_sigma;	 // variation in detection by day (nested within each population)
...
// -----
  ...
  // within-season survival given by a simple population-level intercept ** could increase this complexity eventually 
             phi[t] = inv_logit(inseason_pop[pop_phi[t]]);
	...
	for (t in 1:ind_occ) {
	 if (p_zeros[t] == 0) {       // just a placeholder because stan can't have NA -- these are periods before an animal was captured that doesn't impact the likelihood
	   p[t] = 0;
           ...
           p[t] = inv_logit(p_pop[pop_p[t]] + beta_p[spec_p[t]] * ind_len_scaled[ind_occ_rep[t]]);
	 }
	}  	
