...
// Given long comments, this model is best read in full screen on an external monitor
  ...
  // site-level covariates, categorical
	int<lower=0> pop_drawdown[n_pop];		    // population specific covariate for proportion drawdown    
  	int<lower=0> pop_hydro[n_pop];    
  	int<lower=0> pop_sub[n_pop];      
  	int<lower=0> pop_region[n_pop];   
  	
  // site-by- level covariates, categorical
	int<lower=0> p_drawdown[n_days];   
  	int<lower=0> p_veg[n_days];        
...
mehg_pop[ind_mehg_pop[i]]
	...
	for (z in 1:n_pop) {
	  ...
	  mehg_pop_est[z] = exp(
beta_mehg_spec[spec_pop[z]] + 
beta_mehg_drawdown[pop_drawdown[z]] + 
beta_mehg_hydro[pop_hydro[z]] + 
beta_mehg_sub[pop_sub[z]] + 
beta_mehg_region[pop_region[z]] + 
mehg_pop[z]
);
	} 
