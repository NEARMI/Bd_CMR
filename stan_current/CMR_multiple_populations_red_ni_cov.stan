...
// Given long comments, this model is best read in full screen on an external monitor
  ...
  // short vector indexes (length of n_ind)	
	...
	int<lower=0> ind_which_pop[n_ind];
  ...
  // short vector indexes (length of X_stat_index)
	...
	int<lower=0> pop_for_bd[ind_per_period_bd];
  ...
  // site-level covariates, categorical
	int<lower=0> pop_drawdown[n_pop];

  // site-level covariates, continuous
	real<lower=0> pop_temp[n_pop_year];
...
// -----
	...
	real<lower=0> offseason_pop_bd_sigma;		 // variation in offseason survival by population (intercept)
...
// -----
	...
	for (i in 1:n_ind) {
  	  ...
  	  bd_ind[i] = bd_ind_sigma[ind_which_pop[i]] * bd_ind_eps[i];  
	...
	for (t in 1:ind_per_period_bd) {
	  ...
	  X[t] = (beta_bd + beta_bd_temp * pop_temp[pop_for_bd[t]] + bd_ind[ind_bd_rep[t]] + bd_pop_year[ind_in_pop_year[t]]);      
	 ...
	 if (phi_zeros[t] == 1) {			// phi_zeros is 1 before an individual is caught for the first time
	  ...
	  if (phi_ones[t] == 1) {
	   ...
	   if (offseason[t] == 0) {
             ...
             phi[t] = inv_logit(inseason_pop[pop_phi[t]] + beta_phi * ind_size[ind_occ_min1_rep[t]]);

	   } else {

	     phi[t] = inv_logit(
beta_drawdown[pop_drawdown[pop_phi[t]]] +
offseason_pop[pop_phi[t]] + 
offseason_pop_bd[pop_phi[t]] * X[phi_bd_index[t]] + 
beta_offseason * ind_size[ind_occ_min1_rep[t]]
);
...
// -----
	...
	for (t in 1:ind_occ) {
	 ...
	 if (p_zeros[t] == 1) {				
          p[t] = inv_logit(p_pop[pop_p[t]] + beta_p * ind_size[ind_occ_rep[t]]);
	 } else {
          p[t] = inv_logit(p_pop[pop_p[t]] + beta_p * ind_size[ind_occ_rep[t]]) * gamma[gamma_index[t]];
	 }
...
// -----
	...
	beta_phi       ~ normal(0, 1.2);
	beta_offseason ~ normal(0, 1.2);
	beta_drawdown  ~ normal(0, 1.2);
	...
	for (i in 1:n_pop) {
	  ...
	  offseason_pop_bd_eps[i] ~ normal(0, 3);
