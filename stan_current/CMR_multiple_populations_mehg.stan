...
// -----
  ...
  // fixed
	...
	vector[n_spec] beta_offseason_mehg_bd;		 // interaction term
  ...
  // random: variance
	...
	real<lower=0> offseason_pop_mehg_bd_sigma;	 // variation in interaction term by population
  ...
  // random: deviates
	...
	vector[n_pop] offseason_pop_mehg_bd_eps;	 // pop interaction effect
...
// ------------------------------ transformed parameters ------------------------------
  ...
  // Survival
	...
	vector[n_pop] offseason_pop_mehg_bd;		 // population specific survival between seasons (interaction)
...
// linear predictor for mehg regression	
  	mu_mehg_have   = exp(
ind_spec[ind_mehg_which_have, ] * beta_mehg_spec +
...
// linear predictor for mehg regression
  	mu_mehg_mis   = exp(
ind_spec[ind_mehg_which_mis, ] * beta_mehg_spec +
...
// -----
  ...
  // pop-spec deviates
	for (pp in 1:n_pop) {
	 ...
	 offseason_pop_mehg_bd[pp]  = offseason_pop_mehg_bd_sigma  * offseason_pop_mehg_bd_eps[pp];
	} 
...
(fe_mm_phi_slope * beta_offseason_mehg) .* ind_mehg_scaled[ind_occ_min1_rep[phi_off_index]] +
(fe_mm_phi_slope * beta_offseason_bd   + offseason_pop_bd[pop_phi[phi_off_index]]) .* X[phi_bd_index[phi_off_index]] +
(fe_mm_phi_slope * beta_offseason_len  + offseason_pop_len[pop_phi[phi_off_index]]) .* ind_len_scaled[ind_occ_min1_rep[phi_off_index]] +
(fe_mm_phi_slope * beta_offseason_mehg_bd + offseason_pop_mehg_bd[pop_phi[phi_off_index]]) .* ind_mehg_scaled[ind_occ_min1_rep[phi_off_index]] .* X[phi_bd_index[phi_off_index]]
);
...
// -----
  ...
  // fixed
	...
	beta_offseason_mehg_bd ~ normal(0, 0.65);
  ...
  // variances
	...
	offseason_pop_mehg_bd_sigma ~ inv_gamma(8, 15);
	inseason_pop_sigma      ~ inv_gamma(8, 15);

  // deviates
	inseason_pop_eps      ~ normal(0, 1.75);
	offseason_pop_eps     ~ normal(0, 0.65);
	offseason_pop_bd_eps  ~ normal(0, 0.65);
	offseason_pop_len_eps ~ normal(0, 0.65);
	offseason_pop_mehg_bd_eps ~ normal(0, 0.65);
