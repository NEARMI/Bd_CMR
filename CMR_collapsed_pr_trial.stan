...
// -----
	...
	vector[2] beta_bd;				 // bd fixed effects
...
// -----
	...
	for (t in 1:ind_time) {
	  ...
	  X[t] = (beta_bd[1] + bd_pop[ind_in_pop[t]] + bd_ind[ind_bd_rep[t]]) + beta_bd[2] * temp[t];    
	 ...
	 if (phi_zeros[t] == 1) {			// phi_zeros is 1 before an individual is caught for the first time
	  ...
	  if (offseason[t] == 0) {
		...
		// Can test adding beta_timegaps * time_gaps[t] back into the offseason (where timegaps is from the date till the end of the season)
           ...
           phi[t] = inv_logit(beta_offseason[1] + (beta_offseason[2] + offseason_pop[pop_phi[t]]) * bd_ind[ind_occ_min1_rep[t]] + beta_timegaps * time_gaps[t]);
...
// -----
	...
	offseason_pop_sigma ~ inv_gamma(1, 1);
	p_pop_sigma         ~ inv_gamma(1, 1);

	for (i in 1:n_pop) {
	  bd_pop_eps[i]        ~ normal(0, 3);
	  phi_pop_eps[i]       ~ normal(0, 3);
